...
const BuiltInAIRuleValue = `判断这是不是靠谱的个人转租/出租，而不是中介或灌水。

通过（passed=true）须同时大致满足：
- 像个人在说自己的房子或转租，不是中介话术（没有「随时看房/多套可选/免费带看/直租中介费」这类揽客口吻，也不像复制的广告模板）
- 信息能核对：有明确小区或大致位置，有月租数字，有可联系方式（微信或手机）；缺这三项里的两项就偏不靠谱
- 不是已成交钓鱼、纯求租、合租招室友但写成整租、或明显虚构
...
不要因为帖子里出现「非中介」「个人」几个字就通过，要看整段是否像真人在出租。不确定时宁可拒绝。`
