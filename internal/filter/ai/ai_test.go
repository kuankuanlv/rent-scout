...
// Plan 10 E1：截断统一 rule.DefaultTrimLimit，不再按源读配置 map
func TestAIBatchEvaluatorDefaultTrim(t *testing.T) {
	fl := &fakeLLM{}
	ev := NewAIBatchEvaluator(fl) // 截断统一 rule.DefaultTrimLimit
	long := strings.Repeat("甲", rule.DefaultTrimLimit+50)
	posts := []models.RentPost{
	...
	want := strings.Repeat("甲", rule.DefaultTrimLimit)
	if !strings.Contains(fl.user, want) || strings.Contains(fl.user, strings.Repeat("甲", rule.DefaultTrimLimit+1)) {
		t.Error("各源均应按 rule.DefaultTrimLimit 截断")
	}
}
