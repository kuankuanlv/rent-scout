...
func TestCollectorOnboardDefaultOff(t *testing.T) {
	h := collectorOnboard(config.DefaultApp(), config.DefaultSecrets(), "")
...
func TestCollectorOnboardMissingCookie(t *testing.T) {
	...
	h := collectorOnboard(app, env, "tok")
...
func TestCollectOnboardIncludesAIAndNotify(t *testing.T) {
	hints := collectOnboard(config.DefaultApp(), config.DefaultSecrets(), "")
	var titles []string
	for _, h := range hints {
