...
// 凑批语义：fetch 的 limit 来自 pipeline 的 BatchSize，不得被热配置二次覆盖截短。
...
// 回归防线：库内积压超过配置 BatchSize 时不受限制，抽批上限只由 pipeline 的 limit 决定。
...
// 一致性：pipeline 会用配置的 BatchSize 作为 limit（=10），fetch 返回不超过该上限。
func TestFetchRespectsPipelineLimit(t *testing.T) {
	...
	if len(posts) != 10 {
		t.Fatalf("fetch 应按 pipeline limit=10 返回：len=%d want 10", len(posts))
	}
}
