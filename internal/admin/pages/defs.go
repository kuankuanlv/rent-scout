	...
	return []configSection{
		...
		makeSection("collector", "采集", "豆瓣、微博默认都不开。想采哪个就勾启用，再把浏览器里的 Cookie 整段贴进对应框（直接粘贴内容即可，不用改格式）。也可用 CookieCloud 自动同步。", []configBlock{
			...
			},
			{
				...
				Items: []configField{
					{Key: config.KeyDoubanCookieMode, Label: "Cookie 模式", Value: get(config.KeyDoubanCookieMode, env.Collector.Douban.CookieMode), Type: "select", Options: []string{config.CookieModeNone.String(), config.CookieModeRaw.String(), config.CookieModeCookieCloud.String()}, Hint: "none 不带 cookie；raw 需先登录 https://www.douban.com/ 后复制整段 Cookie 粘贴；cookiecloud 只填三元组", Group: "douban"},
			...
			},
			{
				...
				Items: []configField{
					{Key: config.KeyWeiboCookieMode, Label: "Cookie 模式", Value: get(config.KeyWeiboCookieMode, env.Collector.Weibo.CookieMode), Type: "select", Options: []string{config.CookieModeNone.String(), config.CookieModeRaw.String(), config.CookieModeCookieCloud.String()}, Hint: "none 不带 cookie；raw 需先登录 https://www.weibo.com/ 后复制整段 Cookie 粘贴；cookiecloud 只填三元组", Group: "weibo"},
			...
			},
			{
				...
				Items: []configField{
					{Key: config.KeyXiaohongshuCookieMode, Label: "Cookie 模式", Value: get(config.KeyXiaohongshuCookieMode, env.Collector.Xiaohongshu.CookieMode), Type: "select", Options: []string{config.CookieModeNone.String(), config.CookieModeRaw.String(), config.CookieModeCookieCloud.String()}, Hint: "none 不带 cookie；raw 需先登录 https://www.xiaohongshu.com/ 后复制整段 Cookie 粘贴；cookiecloud 只填三元组", Group: "xiaohongshu"},
